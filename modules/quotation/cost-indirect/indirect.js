...
const IndirectCost = {
    ...
    init() {
        ...
        // ★ 核心改動：模擬從「成本估算表」抓取資料放入甘特圖中
        this.fetchCostEstimationItems();
    ...
    // ★ 新增功能：模擬從「成本估算表」取得設備項目
    fetchCostEstimationItems() {
        // 這裡模擬從系統資料庫或 LocalStorage 抓取該專案的成本估算表項目
        const mockCostItems = [
            { id: 'CE001', name: '主廠房土建設備' },
            { id: 'CE002', name: '無塵室庫板設備' },
            { id: 'CE003', name: '機電空調設備' },
            { id: 'CE004', name: '製程管線設備' },
            { id: 'CE005', name: '消防與工安設備' }
        ];
        ...
        // 轉換為 scheduleItems 格式
        this.data.scheduleItems = mockCostItems.map((item, index) => ({
            id: String.fromCharCode(65 + index), // A, B, C...
            name: item.name,
            // 預設將時間設定為專案起訖時間，使用者可再自行微調
            startMonth: startVal + '-01',
            endMonth: endVal + '-28'
        }));
    },
