/* excel_export.js - 工程系統 Excel 匯出專用模組 V1.5 (修正 maxCols 計算與右側空儲存格框線流失問題) */

const IndirectCostExcelExporter = {
    // 共用常數定義
    CATEGORIES: ['管理人員', '臨時辦公室', '住宿', '交通費', '機具', '整地', '臨時圍籬', '工安設施', '其他'],
    ...
    _execute(context) {
        try {
            ...
            const fileName = `工程間接成本與進度表_${projectInfo.qNo}_${dateStr}.xlsx`;

            const wsMain = this._buildMainSheet(context, projectInfo);
            if (wsMain) XLSX.utils.book_append_sheet(workbook, wsMain, "工程間接成本");

            const wsSchedule = this._buildScheduleSheet(context, projectInfo);
            if (wsSchedule) XLSX.utils.book_append_sheet(workbook, wsSchedule, "工程進度規劃");
    ...
    _cleanTableDom(tableElement, targetKeywordsToRemove) {
        ...
        if (tbody) {
            tbody.querySelectorAll('tr').forEach(row => {
                // 這些是由 JS 動態產生的特殊列，先清空內容避免干擾後續欄數計算
        ...
        // ★ 核心修復：徹底清空原始 tfoot 或舊的表尾，避免遺留的 colspan 撐大 maxCols
    ...
    _buildScheduleSheet(context, projectInfo) {
        ...
        this._insertProjectInfoHeaders(cloneTable.querySelector('thead'), maxCols, '工程進度規劃表', projectInfo);

        // ★ 核心修復：強制把所有空儲存格填入「不換行空白(\u00A0)」以確保框線渲染
        cloneTable.querySelectorAll('th, td').forEach(cell => {
        ...
        if (ws['!ref']) {
            ...
            for (let i = 0; i < maxCols; i++) {
                if (i === 0) colWidths.push({ wch: 6 });       // 項次
                else if (i === 1) colWidths.push({ wch: 28 }); // 工程名稱
                else if (i === 2 || i === 3) colWidths.push({ wch: 12 }); // 日期
                else colWidths.push({ wch: 6 });               // 甘特圖月份正常寬度
            }
            ws['!cols'] = colWidths;
        }
        return ws;
    ...
    _buildMainSheet(context, projectInfo) {
        ...
        // ★ 核心修復：只依據標頭第一行來計算最大的真實欄數 (正確應為 39)
        let maxCols = 0;
        ...
        this._insertProjectInfoHeaders(cloneTable.querySelector('thead'), maxCols, '工程間接成本估算表', projectInfo);
        ...
        // ★ 核心修復：強制把所有空儲存格填入「不換行空白(\u00A0)」，強迫轉換引擎畫出每一格的邊框
        cloneTable.querySelectorAll('th, td').forEach(cell => {
